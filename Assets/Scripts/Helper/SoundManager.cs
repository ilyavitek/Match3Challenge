...
public class SoundManager : MonoBehaviour {
	...
	const float MusicFadeTime = 1.3f;
	...
	public static void PlaySound (string name, bool pausable = true, float volume = 1f) {
		Instance.PlaySoundInternal (name, pausable, volume);
	}
	...
	void PlaySoundInternal (string soundName, bool pausable, float volume) {
		...
		StartCoroutine (PlaySoundInternalSoon (soundName, pausable, volume));
	}
	...
	IEnumerator PlaySoundInternalSoon (string soundName, bool pausable, float volume) {
		...
		float soundVolume = volume == 1 ? DefaultSoundVolume : volume;

		soundSource.volume = _volumeSound * soundVolume;
		soundSource.clip = soundClip;
		soundSource.Play ();
		soundSource.ignoreListenerPause = !pausable;
